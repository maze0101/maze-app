// MAZE APP push сервер (Vercel Function, үнэгүй). Firebase Cloud Functions (Blaze) хэрэггүй.
// Апп зурвас/дуудлага/найзын хүсэлт илгээсний дараа энд POST хийнэ; энд FCM push илгээнэ.
// Vercel → Settings → Environment Variables: SERVICE_ACCOUNT = Firebase service account JSON (бүтнээр нь)
...
const docUrl = path => `https://firestore.googleapis.com/v1/projects/${PROJECT_ID}/databases/(default)/documents/${path}`;
async function fsGet(tok, path) {
  const r = await fetch(docUrl(path), {headers: {Authorization: 'Bearer ' + tok}});
...
async function fsList(tok, path) {
  const r = await fetch(docUrl(path) + '?pageSize=20', {headers: {Authorization: 'Bearer ' + tok}});
  if (!r.ok) return [];
...
async function push(tok, uids, data, ttl) {
  await Promise.all(uids.map(async uid => {
    const names = await fsList(tok, `users/${uid}/fcm`);
...
async function handle(req) {
  ...
  if (b.kind === 'msg') {
    ...
    const c = await fsGet(tok, 'chats/' + cid);
    if (!c || !(c.members || []).includes(uid)) return 403;
    const to = [];
    for (const u of (c.members || []).filter(x => x !== uid)) {
      if (!(await fsGet(tok, `blocks/${u}_${uid}`))) to.push(u); // хүлээн авагч илгээгчийг блоклоогүй
    }
    const title = c.group ? short(c.name, 40) : myName;
    const body = (c.group ? short(myName, 30) + ': ' : '') + short(b.preview, 120);
    await push(tok, to, {type: 'msg', title: '✉ ' + title, body, tag: 'chat-' + cid, url: './index.html'}, 3600);
    return 200;
  }
  if (b.kind === 'call') {
    const id = String(b.id || '');
    const c = await fsGet(tok, 'calls/' + encodeURIComponent(id));
  ...
  if (b.kind === 'gcall') {
    const id = String(b.id || '');
    const c = await fsGet(tok, 'gcalls/' + encodeURIComponent(id));
    if (!c || c.from !== uid || c.status !== 'active') return 403;
    if (Date.now() - Date.parse(c.createdAt || 0) > 120e3) return 403;
    const members = (c.members || []).filter(x => x !== uid);
    const blocked = await Promise.all(members.map(u => fsGet(tok, `blocks/${u}_${uid}`)));
    await push(tok, members.filter((_, i) => !blocked[i]), {
  ...
  if (b.kind === 'req') {
    const id = String(b.id || '');
    const r = await fsGet(tok, 'requests/' + encodeURIComponent(id));
...
module.exports = async (req, res) => {
  ...
  if (req.method !== 'POST') return res.status(200).send('maze-push ok');
  let status = 500;
  try { status = await handle(req); } catch (e) { status = /token|alg|kid/.test(e.message) ? 401 : 500; }
